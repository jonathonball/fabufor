...
class LuxColor extends EventEmitter {
     ...
     */
    _distanceToMove(current, target) {
     ...
     */
    _adjustVector(value, target) {
        if (value < target) {
            return value + this._distanceToMove(value, target);
        }
        if (value > target) {
            return value - this._distanceToMove(value, target);
        }
    ...
    setColors(colors) {
        if (! colors.length) {
            console.error('Please provide a sequence of color names as positional parameters');
            process.exit(1);
        }
        this.colors = colors;
    }
    ...
    targetNextSequence() {
        let rgb = hexRgb(this.colors[this.index].hex);
        this.targetR = rgb.red;
        this.targetG = rgb.green;
        this.targetB = rgb.blue;
        this.targetCode = this._rgbString(this.targetR, this.targetG, this.targetB);
        this.targetName = this._colorName(this.targetR, this.targetG, this.targetB);
     ...
     */
    update() {
        this.r = this._clamp(this._adjustVector(this.r, this.targetR));
        this.g = this._clamp(this._adjustVector(this.g, this.targetG));
        this.b = this._clamp(this._adjustVector(this.b, this.targetB));
    ...
    colorNameToRgb(name) {
        let results = namer(name, { pick: ['html'] });
        if (results.html && results.html[0].name) {
            return results.html[0];
        }
        return {};
    }
