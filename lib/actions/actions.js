const {Luxafor, API} = require('node-luxafor2');
...
class Actions {
    ...
    getKey(keys, k) {
        return Luxafor.getApiValue(keys, k);
    }
    ...
    getLed(led) {
        ...
        let result = this.getKey('led', led);
        if (! result) {
    ...
    getPresetColor(color) {
        let result = this.getKey('color', color);
        if (! result) {
    ...
    getWaveType(wave) {
        let result = this.getKey('wave_type', wave);
        if (! result) {
    ...
    /**
     * Checks that a value is a color and failing that; freak out
     * @param {String} userColor - Value representing a string
     * @return {tinycolor}
     */
